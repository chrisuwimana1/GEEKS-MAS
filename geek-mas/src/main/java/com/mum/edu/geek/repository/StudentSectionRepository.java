...
public interface StudentSectionRepository extends JpaRepository<StudentSection, Integer> {
    @Query(value = "select ss from StudentSection ss where ss.studentFacultyId.studentId = :studentId")
    public List<StudentSection> findByStudentId(Integer studentId);
    public List<StudentSection> findByBlockId(Integer blockId);
    @Query(value = "select ss from StudentSection ss where ss.studentFacultyId.studentId = :studentId and ss.blockId = :blockId")
